...
def create_activity_log(
    ...
    return repo.create_activity_log(activity_log_create=log_data, user_id=current_user.id)
...
def read_activity_logs(
    ...
    return repo.get_all_activity_logs_by_user(user_id=current_user.id)
...
def read_activity_log(
    ...
    log = repo.get_activity_log_by_id(log_id=log_id, user_id=current_user.id)
    if not log:
...
def update_activity_log(
    ...
    if not log:
        raise HTTPException(status_code=404, detail="Activity log not found")
    return repo.update_activity_log(log=log, log_update=log_data)
...
def delete_activity_log(
    ...
    if not log:
        raise HTTPException(status_code=404, detail="Activity log not found")
    repo.delete_activity_log(log=log)
    return
